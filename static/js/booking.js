let booking_model = {
    ...
    del_status: null,
    main(){
        return fetch('/static/booking_main.html').then(res=>res.text()).then(data=>{
            this.main_html = data;
        })
    },
    get_data(){
...
let booking_view = {
    ...
    init(data){
        if(!data) return el("booking_data").innerHTML = "目前沒有任何待預定的行程";
...
let booking_controller = {
    init: async function(){
        ...
        await booking_model.main();
        await booking_view.init_main(booking_model.main_html);
